...
2. **인쇄/PDF 결과물 최종 확인** — 표 폭이 페이지를 넘어가서 잘리는 버그는 2026-08-11에 발견·수정·배포 완료(6.3 참고). 대표님이 실제로 인쇄/PDF 저장 버튼을 한 번 눌러서 눈으로 최종 확인하면 좋음(자동화로는 네이티브 인쇄창 자체를 열어볼 수 없어서 로직 검증까지만 함).
