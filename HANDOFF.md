...
C. **시공 사례를 공간별로 나누기** — 아직 미착수. 현재 `projects` 테이블의 `category`는 아파트/주택/기타 3종류뿐(`admin/portfolio.html`), PROJECTS 페이지엔 이 3개 기준 필터 탭이 이미 있음(2026-08-18 리디자인 때 추가). "공간별"이 거실/주방/욕실처럼 더 세분화된 분류를 말하는 건지 다음 세션에 먼저 확인하고 진행할 것. 아래 12번의 "프로젝트 상세페이지" 작업과 자연스럽게 묶어서 진행하면 좋음.
D. **디오브(diov.kr) 스타일 프로젝트 상세페이지** — 아직 미착수. PROJECTS 그리드에서 카드 클릭 시 지금은 아무 반응 없음. 디오브는 클릭하면 사진 세로 스크롤 갤러리 + 우측에 Project/Type/Location/Floor Area/Director/Design 메타정보 패널이 뜸. DB에 위치·평수·담당자 등 필드 추가 필요(현재 `projects` 테이블엔 title/category/description/image_url/sort_order뿐).
E. **나머지 시공 사진 포트폴리오 등록 여부 확인 필요** — 대표님이 홈페이지 히어로용으로 사진 11장을 주셨고 전부 `assets/hero-1.jpg`~`hero-11.jpg`로 홈 캐러셀에 들어갔음. Claude Code가 "이 사진들 PROJECTS에도 시공사례로 등록해줄까요?"라고 두 번 물어봤는데 아직 답을 못 받음 — 다음 세션에 먼저 물어보고, 등록하려면 각 사진이 어느 현장(위치/평수)인지부터 확인해야 함.
