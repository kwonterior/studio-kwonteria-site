...
**다음 세션 최우선 (2026-08-17에 대표님이 직접 지정, "일단 이거부터"):**

A. **홈페이지 글씨체** — 폰트 교체/개선. 현재 `css/style.css`의 `body` font-family는 시스템 기본(`-apple-system, BlinkMacSystemFont, "Malgun Gothic", "Apple SD Gothic Neo", sans-serif`), 별도 웹폰트 없음. 어떤 느낌(고급스러운/모던한/부드러운 등)을 원하시는지, 참고 사이트가 있는지부터 확인하고 시작할 것.
B. **회사 로고 및 글씨체 수정** — 현재 로고는 이미지가 아니라 텍스트 워드마크(`index.html`의 `<a class="logo">스튜디오 <span>권테리어</span></a>`, `css/style.css` `.logo`). 실제 로고 이미지가 있는지, 아니면 폰트/스타일만 다시 잡을지 확인 필요.
C. **시공 사례를 공간별로 나누기** — 현재 `projects` 테이블의 `category`는 아파트/주택/기타 3종류뿐(`admin/portfolio.html`). "공간별"이 거실/주방/욕실/침실 같은 공간 단위 분류를 말하는 건지, 홈페이지 PROJECTS 화면에 필터 탭을 추가하자는 건지 다음 세션에 먼저 확인하고 진행할 것.
